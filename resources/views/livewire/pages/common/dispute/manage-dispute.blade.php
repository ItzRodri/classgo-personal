        ...
        <ul class="am-dispute-list">
          ...
          <li class="am-dispute-item">
            ...
            <div class="am-dispute-user">
              <figure>
                @if (!empty($dispute?->responsibleBy?->profile?->image) && Storage::disk('public')->exists($dispute?->responsibleBy?->profile?->image))
                <img src="{{ resizedImage($dispute?->responsibleBy?->profile?->image,34,34) }}" alt="{{$dispute?->responsibleBy?->profile?->image}}" />
                @else
                    <img src="{{ setting('_general.default_avatar_for_user') ? asset('storage/' . setting('_general.default_avatar_for_user')[0]['path']) : resizedImage('placeholder.png', 34, 34) }}" alt="{{ $dispute?->responsibleBy?->profile?->image }}" />
                @endif
              </figure>
              <div>
          ...
          <li class="am-dispute-item">
            ...
            <div class="am-dispute-user">
              <figure>
                ...
                @else
                    <img src="{{ setting('_general.default_avatar_for_user') ? asset('storage/' . setting('_general.default_avatar_for_user')[0]['path']) : resizedImage('placeholder.png', 34, 34) }}" alt="{{ $dispute?->creatorBy?->profile?->image }}" />
                @endif
              </figure>
              <div>
      ...
      <div class="am-user-profile">
        ...
        <div class="am-user-card">
          <div class="am-user-avatar">
            @if (!empty($admin?->profile?->image) && Storage::disk('public')->exists($admin?->profile?->image))
                <img src="{{ resizedImage($admin?->profile?->image,34,34) }}" alt="{{$admin?->profile?->image}}" />
            @else
                <img src="{{ setting('_general.default_avatar_for_user') ? asset('storage/' . setting('_general.default_avatar_for_user')[0]['path']) : resizedImage('placeholder.png', 34, 34) }}" alt="{{ $admin?->profile?->image }}" />
            @endif
          </div>
      ...
      <div class="am-chat-messages am-custom-scrollbar-y">
        ...
        <div class="am-message {{ $chat->user_id == $user->id ? 'am-reply-message' : '' }}">
          <div class="am-message-avatar">
            @if (!empty($chat->user?->profile?->image) && Storage::disk('public')->exists($chat->user?->profile?->image))
                <img src="{{ resizedImage($chat->user?->profile?->image,34,34) }}" alt="{{$chat->user?->profile?->image}}" />
            @else
                <img src="{{ setting('_general.default_avatar_for_user') ? asset('storage/' . setting('_general.default_avatar_for_user')[0]['path']) : resizedImage('placeholder.png', 34, 34) }}" alt="{{ $chat->user?->profile?->image }}" />
            @endif
          </div>
