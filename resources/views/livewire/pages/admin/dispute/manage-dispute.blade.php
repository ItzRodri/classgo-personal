      ...
      <div class="am-user-profile">
        ...
        @if($withChat=='student')
            ...
            <div class="am-user-avatar">
                ...
                <img src="{{ resizedImage($dispute?->creatorBy?->profile?->image,34,34) }}" alt="{{$dispute?->creatorBy?->profile?->image}}" />
                @else
                    <img src="{{ setting('_general.default_avatar_for_user') ? asset('storage/' . setting('_general.default_avatar_for_user')[0]['path']) : resizedImage('placeholder.png', 34, 34) }}" alt="{{ $userChat[0]->user?->profile?->image }}" />
                @endif
            </div>
